...
export class Task4 implements Contract {
    ...
    async getCaesarCipherEncrypt(provider: ContractProvider, shift: bigint, text: Cell) {
        ...
        params.writeNumber(shift);
        params.writeCell(text);
        return await provider.get("caesar_cipher_encrypt", params.build());
    }
    ...
    async getCaesarCipherDecrypt(provider: ContractProvider, shift: bigint, text: Cell) {
        ...
        params.writeNumber(shift);
        params.writeCell(text);
        return await provider.get("caesar_cipher_decrypt", params.build());
    }
}
